...
| `MLFLOW_TRACKING_URI` | `http://localhost:5000` | MLflow tracking server (`.env.example` ships `5002` as a safe pre-emptive default) |
...
| MLflow UI | http://localhost:5000 (falls back to `5002+` if port is busy) |

> The `.env.example` ships with `MLFLOW_TRACKING_URI=http://localhost:5002` as a pre-emptive safe default to avoid common port conflicts. Adjust it to match the port reported at startup.
...
┌──────────────────────────────────────────────────────────────┐
│                       Observability                           │
│  MLflow autolog  →  http://localhost:5000  (traces + runs)   │
...
Open the MLflow UI at **http://localhost:5000** (or whichever port was assigned at startup) and navigate to the `ghost-contributors-demo` experiment to see:
