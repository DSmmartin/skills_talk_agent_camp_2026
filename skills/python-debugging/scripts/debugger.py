#!/usr/bin/env python3
"""
Python Skill Debugger

A PyCharm-like debugging experience for Codex skill usage with breakpoints,
...
- Session state stored in current working directory `.skill_python_debugger/`
"""
...
# Session directory
SESSION_DIR = Path(os.path.curdir) / ".skill_python_debugger"
SOCKET_TIMEOUT = 30.0
EVAL_TIMEOUT = 5
...
def format_value(
    obj: Any, max_depth: int = 2, current_depth: int = 0, seen: Optional[set] = None
) -> Dict[str, Any]:
    ...
    try:
        ...
        if isinstance(obj, (list, tuple)):
            ...
            for i, item in enumerate(obj):
                if i >= MAX_COLLECTION_ITEMS:
                    items.append(
                        {"type": "...", "value": f"... ({len(obj) - i} more items)"}
                    )
                    break
                items.append(
                    format_value(item, max_depth, current_depth + 1, seen.copy())
                )
        ...
        if isinstance(obj, dict):
            ...
            for i, (k, v) in enumerate(obj.items()):
                ...
                items[key_str] = format_value(
                    v, max_depth, current_depth + 1, seen.copy()
                )
...
# =============================================================================
# Python Debugger (bdb.Bdb Extension)
# =============================================================================


class PythonDebugger(bdb.Bdb):
    """Custom debugger extending bdb.Bdb for skill integration."""
    ...
    def run_script(
        ...
        work_dir = os.path.abspath(cwd) if cwd else script_dir

        # Set up sys.argv
        sys.argv = [script_path] + args

        # Add working dir and script dir to path so both relative imports and
        # the script itself resolve correctly
        for p in (work_dir, script_dir):
        ...
        # Load .env file: explicit path > <cwd>/.env > skip
        dot_env_path = None
        if env_file:
            dot_env_path = os.path.abspath(env_file)
        else:
            candidate = os.path.join(work_dir, ".env")
            if os.path.exists(candidate):
                dot_env_path = candidate

        if dot_env_path:
            self._load_env_file(dot_env_path)
    ...
    def _cmd_delete(self, command: Dict) -> Dict:
        ...
        if exception_type:
            if exception_type == "*":
                ...
                return {
                    "status": "ok",
                    "message": f"Exception breakpoint for {exception_type} removed",
                }
            else:
        ...
        if filename and lineno:
            ...
            return {
                "status": "ok",
                "message": f"Breakpoint at {filename}:{lineno} deleted",
            }
    ...
    def _cmd_locals(self, command: Dict) -> Dict:
        ...
        return {"status": "ok", "locals": format_variables(frame.f_locals, max_depth)}
    ...
    def _cmd_globals(self, command: Dict) -> Dict:
        ...
        max_depth = command.get("depth", 2)
        return {"status": "ok", "globals": format_variables(frame.f_globals, max_depth)}
...
def cmd_start(args) -> int:
    ...
    if pid == 0:
        ...
        try:
            ...
            debugger = PythonDebugger(session)
            debugger.run_script(
                script_path,
                args.args,
        ...
        if client.connect(timeout=5.0):
            ...
            client.close()
            print(
                json.dumps(response if response else {"status": "started", "pid": pid})
            )
        else:
...
def main():
    ...
    start_parser.add_argument(
        ...
        "sets cwd for relative imports and auto .env discovery",
    )
    start_parser.add_argument(
        "--env-file",
        dest="env_file",
        help="Path to .env file to load (default: <cwd>/.env if it exists)",
    )
    ...
    break_parser.add_argument(
        "-e", "--exception", help="Exception type (use * for all)"
    )
    ...
    locals_parser.add_argument(
        "-d", "--depth", type=int, default=2, help="Inspection depth"
    )
    ...
    inspect_parser.add_argument(
        "-d", "--depth", type=int, default=4, help="Inspection depth"
    )
